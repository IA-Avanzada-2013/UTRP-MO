...
double * allocateMemoryArrayDoubles(int size){
	double *array = NULL;
...
void deallocateMemoryArrayDoubles(double *array){
	if (array != NULL ){
		free(array);
	}
}
...
void obtainReferencePoints(Problem *pbm, double *ref){
	...
	if (strncmp (pbm->get_name(),"Mandl",sizeof("Mandl"))== 0){
		ref[0] = 16; //15.1304;
		ref[1] = 225; //224;
	}
	else if(strncmp (pbm->get_name(),"Mumford0",sizeof("Mumford0"))== 0){
		ref[0] = 33; //32.3961;
		ref[1] = 760; //759;
	}
	else if(strncmp (pbm->get_name(),"Mumford1",sizeof("Mumford1"))== 0){
		ref[0] = 35; //34.688;
		ref[1] = 2039; //2038;
	}
	else if(strncmp (pbm->get_name(),"Mumford2",sizeof("Mumford2"))== 0){
		ref[0] = 37; //36.5416;
		ref[1] = 5633; //5632;
	}
	else if(strncmp (pbm->get_name(),"Mumford3",sizeof("Mumford3"))== 0){
		ref[0] = 37; //36.9159;
		ref[1] = 6666; //6665;
	}
	else{
	  std::cout<<"Problem unreachable!";
	  return;
	}
	return;
}
...
double hv(SolutionSet *pop, Problem *pbm){
	int nobj=pbm->nobj;
	int npts=SolutionSet->solutions.size();
	double *ref;
	ref = allocateMemoryArrayDoubles(pbm->nobj);
	obtainReferencePoints(pbm, ref);
	double *data, value;
	int i, k, j;
	data = allocateMemoryArrayDoubles(npts*nobj);

	j = 0;
	for (i = 0; i < npts; i++) {
		//for (k = 0; k < nobj; k++)
		data[j] = pop->solutions[i].fo1;
		j++;
		data[j] = pop->solutions[i].fo2;
		j ++;
	}
	...
	deallocateMemoryArrayDoubles(data);
	return value;
}
